        ...
        this.customTools = options?.tools || [
            {
                ...
                color: '#409eff',
                children: [
                    { id: 'hello', label: 'bug', icon: IconDocument }
                ]
            },
            {
                id: '2', label: '从三个选项中发现一个', icon: IconDocument, color: '#67c23a', children: [
                    { id: '2-1', label: '喝一口水', details: "补充水分，保持身体水分平衡，提高工作效率" },
                    { id: '2-2', label: '伸个懒腰', details: "缓解肌肉紧张，促进血液循环，减轻久坐带来的疲劳" },
                    { id: '2-3', label: '眺望窗外', details: "放松眼部肌肉，缓解视疲劳，调节心情，提高工作效率" }
                ]
            }
        ];
    ...
    private async convertBookmarksToTools(): Promise<Tool[]> {
        try {
            ...
            const bookmarkTools: Tool[] = bookmarks.map((bookmark: Bookmark) => ({
                ...
                label: bookmark.text.length > 30 ? bookmark.text.substring(0, 28) + '...' : bookmark.text,
                icon: IconBookmark,
                color: '#f59e0b',
    ...
    public async inject(options?: SideBarOptions): Promise<void> {
        try {
            ...
            if (!this.isInjected) {
                ...
                injectCssDom(this.shadowRoot as ShadowRoot, getAssetsAbstractPathSync("css/sidebar"));
                this.isInjected = true;
            }

            if (!this.vueContainer && !this.shadowRoot?.getElementById('shadow-app-sidebar')) {
