...
class FloatingBall implements AppModule {
    ...
    private customTools: Tool[] = [
        { id: 'glass-card', label: '悬浮毛玻璃卡片', details: '创建一个可拖动、可调宽高和透明度的毛玻璃悬浮卡片，双击空白区域即可关闭' },
        { id: 'image', label: '下载图片', details: '下载当前页面的所有图片，包括base64编码的图片，src属性中的图片和svg矢量图' },
        { id: 'mock', label: '构造数据', details: '构造模拟数据，支持构造数组、对象、字符串、数字等类型' },
        { id: 'brute', label: '密码爆破', details: '对密码进行爆破，支持字典攻击、规则攻击等' },
        { id: 'crypto', label: '加解密', details: '对数据进行加解密，支持AES、DES等加密算法' },
        { id: 'json', label: 'JSON格式化', details: '格式化JSON数据，支持缩进、换行等' },
    ...
    async inject(options?: FloatingBallOptions): Promise<void> {
        try {
            if (window.self !== window.top) return maLogger.log('不是主页面，不注入悬浮球'), void 0;
            ...
            if (!this.isInjected) {
                injectCssDom(this.shadowRoot as ShadowRoot, getAssetsAbstractPathSync('css/floatingball'));
                this.isInjected = true;
            }

            if (!this.vueContainer && !this.shadowRoot?.getElementById('shadow-app-floatingball')) {
