 ...
 */
export function injectStyles(shadowRoot: ShadowRoot, styles: string, replaceRootSelector: boolean = false) {
...
export async function injectCssDom(shadowRoot: ShadowRoot, cssUrl: string) {
  try {
    ...
    maLogger.error('Error reading and injecting CSS:', error, `CSS URL: ${cssUrl}`);
    throw error;
  }
}
