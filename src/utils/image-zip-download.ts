...
const fetchImageBlob = async (src: string): Promise<Blob> => {
  if (src.startsWith('data:image')) return fetch(src).then(r => r.blob());
  const res = await fetch(src);
  if (!res.ok) throw new Error(`fetch error ${res.status}`);
  return res.blob();
};

const buildImageInfo = (src: string, idx: number, element: Element, alt: string, prefix = 'image'): ImageInfo => {
  ...
  return {
    ...
    isBase64,
    element,
    name: isBase64 ? `base64_${prefix}_${idx + 1}.${ext}` : getFileName(src, idx, prefix)
  };
};

const scanImgTags = (root: ParentNode): ImageInfo[] =>
  Array.from(root.querySelectorAll('img')).reduce((list, img, idx) =>
    img.src ? (list.push(buildImageInfo(img.src, idx, img, img.alt, 'image')), list) : list,
    [] as ImageInfo[]
  );

const scanBackgroundImages = (root: ParentNode, startIdx: number): ImageInfo[] => {
  const list: ImageInfo[] = [];
  let bgIdx = startIdx;
  root.querySelectorAll('*').forEach(el => {
    const urlMatch = window.getComputedStyle(el).backgroundImage.match(/url\(["']?(.*?)["']?\)/);
    urlMatch?.[1] && list.push(buildImageInfo(urlMatch[1], bgIdx++, el, 'background-image', 'bg'));
  });
  return list;
};
 ...
 */
export async function downloadAllImages(images: ImageInfo[], opt: DownloadOptions = {}): Promise<void> {
 ...
 * @returns 文件名
 */
function getFileName(url: string, idx: number, prefix: string = 'image'): string {
  // 尝试从URL中提取有意义的文件名
 ...
 * @returns Promise<void>
 */
export async function downloadSingleImage(src: string, name: string | null = null): Promise<void> {
  try {
