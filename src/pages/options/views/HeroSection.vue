<template>
  <section ref="bridgeRoot" class="bridge-console" :style="parallaxStyle" @pointermove="handlePointerMove"
    ...
    <div ref="bridgeLayout" class="bridge-layout">
      <svg v-if="enableHoverGuide && hoverGuide && layoutSize.width > 0 && layoutSize.height > 0" class="bridge-layout__overlay"
        :viewBox="`0 0 ${layoutSize.width} ${layoutSize.height}`" aria-hidden="true">
    ...
    <!-- <footer class="quick-dock">
      <button type="button" class="quick-dock__btn quick-dock__btn--primary" @click="emit('open-overview')">
        打开总览
      </button>
      <button type="button" class="quick-dock__btn" @click="emit('navigate-panel', 'up')">
        哨兵监控
      </button>
      <button type="button" class="quick-dock__btn" @click="emit('navigate-panel', 'down')">
        航域许可
      </button>
      <button type="button" class="quick-dock__btn" @click="emit('navigate-panel', 'left')">
        舰体设置
      </button>
      <button type="button" class="quick-dock__btn" @click="emit('navigate-panel', 'right')">
        船员档案
      </button>
    </footer> -->
  </section>
