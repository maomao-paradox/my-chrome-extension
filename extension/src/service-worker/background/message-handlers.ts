...
const createTabWithScript = async (
  payload: CreateTabWithScriptPayload | undefined,
): Promise<{
  tabId: number;
  scriptPath: string;
...
const appendRecordedStep = async (
  ...
  return {
    ...
    AUTOMATION_RUN_STEP: async (payload, _sender, sendResponse) => {
      try {
        ...
        const result = await runStep(tabId, step, {
          allowRisky: payload?.allowRisky === true,
        });
        sendResponse?.({ success: true, payload: result });
      } catch (error) {
        sendResponse?.({
    ...
    AUTOMATION_RUN_STEPS: async (payload, _sender, sendResponse) => {
      try {
        ...
        const steps = Array.isArray(payload?.steps)
          ? (payload.steps as AutomationStep[])
          : [];
        if (steps.length === 0) {
          sendResponse?.({ success: false, error: "缺少 steps" });
          return true;
        }
        const result = await runSteps(tabId, steps, {
          allowRisky: payload?.allowRisky === true,
        });
        sendResponse?.({ success: true, payload: result });
      } catch (error) {
    ...
    AUTOMATION_RECORDED_STEPS_CLEAR: async (
      _payload,
      _sender,
      sendResponse,
    ) => {
    ...
    OPEN_SITE_FAVORITE: async (payload, _sender, sendResponse) => {
      try {
        const url = typeof payload?.url === "string" ? payload.url.trim() : "";
        if (!/^https?:\/\//i.test(url)) {
          sendResponse?.({ success: false, error: "收藏站点 URL 无效" });
          return true;
        }
        await chrome.tabs.create({ url, active: true });
        sendResponse?.({ success: true });
      } catch (error) {
        console.error("打开收藏站点失败:", error);
        sendResponse?.({
          success: false,
          error: error instanceof Error ? error.message : String(error),
        });
    ...
    PAGE_DEVTOOLS_MESSAGE: (payload, sender, sendResponse) => {
      try {
        ...
        if (tabId) {
          const success = devToolsPortManager.sendToDevTools(
            tabId,
            devToolsMessage,
          );
          ...
          const success =
            devToolsPortManager.sendToAllDevTools(devToolsMessage);
          sendResponse?.({ success });
        }
    ...
    LOAD_CONTENT_SCRIPT: (payload, sender, sendResponse) => {
      try {
        ...
        console.log(
          `准备注入脚本: ${data.scriptName}, 路径: ${data.scriptSrc}, 目标标签: ${sender.tab.id}`,
        );
