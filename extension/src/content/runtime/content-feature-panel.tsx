...
interface TourStep {
  title: string;
  description: string;
  target: "header" | "list" | "footer";
}

const TOUR_STEPS: TourStep[] = [
  {
    title: "欢迎使用 Radius 配置",
    description: "这里可以按需管理当前内容脚本的功能。引导完成后，你可以随时使用快捷键重新打开面板。",
    target: "header",
  },
  {
    title: "选择需要的功能",
    description: "每一行代表一个独立业务功能。首次使用时全部关闭，请打开你需要的功能。",
    target: "list",
  },
  {
    title: "保存并刷新页面",
...
const Panel: React.FC<ContentFeaturePanelOptions & { onClose: () => void }> = ({
  ...
  useLayoutEffect(() => {
    ...
    const refs = { header: headerRef, list: listRef, footer: footerRef };
    const updateRect = () => setTargetRect(refs[TOUR_STEPS[tourStep].target].current?.getBoundingClientRect() || null);
    updateRect();
  ...
  return (
    <div className="kn-panel" role="dialog" aria-modal="true" aria-label={`${scriptName} 内容脚本配置`}>
      <div className="kn-panel__header" ref={headerRef}>
        <div>
          <div className="kn-panel__eyebrow">内容脚本配置</div>
          <h2>{scriptName}</h2>
        </div>
        <button className="kn-icon-button" type="button" aria-label="关闭" title="关闭" onClick={onClose}>×</button>
      </div>

      {isFirstUse && (
        <div className="kn-panel__guide">
          <strong>首次使用</strong>
          <span>已为你关闭全部功能。选择需要启用的功能并保存，之后可使用 {shortcut} 再次打开面板。</span>
        </div>
      )}
      ...
      <div className="kn-panel__list" ref={listRef}>
        {features.map((feature) => (
          <MASwitch
            ...
            onChange={(checked) => setDraft((current) => ({ ...current, [feature.id]: checked }))}
            openText="开"
            closeText="关"
          >
      ...
      <div className="kn-panel__footer" ref={footerRef}>
        <button className="kn-button kn-button--ghost" type="button" onClick={onClose}>取消</button>
        <button className="kn-button kn-button--primary" type="button" onClick={() => { onSave(draft); onClose(); }}>保存</button>
      </div>
      ...
      {currentTourStep && targetRect && (
        ...
        >
          ...
          <div
            ...
            style={{
              top: tourStep === TOUR_STEPS.length - 1 ? targetRect.top - 174 : targetRect.bottom + 16,
              left: Math.max(16, Math.min(window.innerWidth - 346, targetRect.left)),
            }}
          >
            <div className="kn-tour__progress">{tourStep + 1} / {TOUR_STEPS.length}</div>
            <h3>{currentTourStep.title}</h3>
            <p>{currentTourStep.description}</p>
            <div className="kn-tour__actions">
              <button className="kn-tour__skip" type="button" onClick={closeTour}>跳过</button>
              {tourStep > 0 && <button className="kn-button kn-button--ghost" type="button" onClick={() => setTourStep(tourStep - 1)}>上一步</button>}
              <button className="kn-button kn-button--primary" type="button" onClick={() => tourStep === TOUR_STEPS.length - 1 ? closeTour() : setTourStep(tourStep + 1)}>{tourStep === TOUR_STEPS.length - 1 ? "完成" : "下一步"}</button>
            </div>
          </div>
        </div>
      )}
...
export const createContentFeaturePanel = (options: ContentFeaturePanelOptions): void => {
