...
import { getAssetsAbstractPath, getAssetsAbstractPathSync } from '@/utils';
...
export const createMessageHandlers = (
    ...
    SCRIPT_INJECTION_ERROR: (data, sendResponse) => {
        ...
        sendResponse({ success: false, message: '脚本注入失败', error: data.error });
        return false;
    },
    ...
    CONFIG_INIT: (_data, sendResponse) => {
        ...
        storage.ext.local.get(configKey)
            .then((result) => {
                try {
                    sendResponse({
    ...
    CONFIG_UPDATE: (data) => {
        void moduleManager.applyConfig(data);
        return false;
    },
    ...
    UPDATE_SIDEBAR_TOOLS: async (data, sendResponse) => {
        ...
        try {
            ...
            sendResponse({ success: false, msg: '更新侧边栏工具失败: ' + (error as Error).message });
        }
        return true;
    },
    ...
    UPDATE_TOOLBAR_TOOLS: async (data, sendResponse) => {
        ...
        try {
            ...
            const updated = await moduleManager.updateModuleTools('textSelectionToolbar', tools);
            sendResponse(updated
                ? { success: true, msg: '文字工具已更新' }
                : { success: false, msg: '文字工具模块未正确加载或缺少更新方法' });
        } catch (error) {
            maLogger.error('更新文字工具失败:', error);
            sendResponse({ success: false, msg: '更新文字工具失败: ' + (error as Error).message });
        }
        return true;
    },
    ...
    CALCULATE_POW: async (data, sendResponse) => {
        ...
        try {
            ...
            sendResponse({ success: false, msg: '计算 POW 失败: ' + (error as Error).message });
        }
        return true;
    },
    ...
    TRIGGER_COMPONENT_CAPTURE: async (_data, sendResponse) => {
        ...
        try {
            ...
            sendResponse({ success: false, msg: '触发组件捕获失败: ' + (error as Error).message });
        }
        return true;
    },
    ...
    "extractAccessibilityTree": async (_data, sendResponse) => {
        try {
            const utilsModule = await import(getAssetsAbstractPathSync('js/runtime/accessibility-utils'));
            const utils = utilsModule.default || utilsModule;

            const options = _data || {};
            const axTree = await utils.extractAccessibilityTree(document.body, options);
