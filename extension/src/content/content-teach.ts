  ...
  const handleRequest = (_R: Function, ...args: any) => {
    ...
    handleResponse = (res: Response<any>, condition?: boolean) => {
      ...
      if ((res.code && res.code === 1e4) || condition) {
        return (
          msg && ctx.message.success(msg as string, timeout),
          res.data || res
        );
      } else {
    ...
    parasitism = () => {
      ...
      location.hash.match("#/login") &&
        waitForSelector({
          selector:
            "#app > div > div > div.login-form-container > form > div.login-form-title",
