...
const buildModuleStates = (
  telemetry: Record<StarshipPanelId, ModuleTelemetry>,
): StarshipModuleState[] => {
  ...
  return STARSHIP_MODULES.map((module) => ({
    ...module,
    telemetry:
      module.id === StarshipPanelId.Main
        ? {
            status:
              warningCount > 0
...
const loadTelemetry = async (): Promise<
...
> => {
  ...
  try {
    ...
    >;
    const themeColor =
      typeof snapshot.themeColor === "string" ? snapshot.themeColor : "#409EFF";
    const language =
    ...
    next.right = {
      ...
      headline:
        totalUsers === 0 ? "暂无船员档案" : `${enabledUsers} 名船员处于启用态`,
      detail:
        totalUsers === 0
          ? "进入模块创建首个自动登录用户"
    ...
    const enabledRoutes = domainEntries.filter(
      (config) => typeof config === "string" || config?.enabled !== false,
    ).length;
    next.bottom = {
      status: enabledRoutes > 0 ? StarshipStatus.Online : StarshipStatus.Standby,
    ...
    const rules = safeJsonParse<unknown[]>(
    ...
    next["top-left"] = {
      status: enabledRules > 0 ? StarshipStatus.Online : StarshipStatus.Standby,
