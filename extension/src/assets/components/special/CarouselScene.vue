...
function rotateCarousel() {
  ...
  const angle = theta.value * selectedIndex.value * -1;
  carousel.value.style.transform = 'translateZ(' + -radius.value + 'px) ' +
        rotateFn.value + '(' + angle + 'deg)';
    
  // 当只有2个卡片时，确保只有正面的卡片是不透明的
  const cells = carousel.value.querySelectorAll('.carousel__cell') as NodeListOf<HTMLElement>;
  const cellCount = Array.isArray(props.cells) ? props.cells.length : props.cells;
  ...
  if (cellCount === 2) {
    for (let i = 0; i < cells.length; i++) {
      ...
      if (i < cellCount) {
        // 计算当前卡片的角度
        const cellAngle = theta.value * i;
        // 计算与当前旋转角度的差值
        const angleDiff = Math.abs((angle + cellAngle) % 360);
...
onMounted(() => {
  ...
  const cells = carousel.value.querySelectorAll('.carousel__cell') as NodeListOf<HTMLElement>;
  const cellCount = Array.isArray(props.cells) ? props.cells.length : props.cells;
  const cellWidth = carousel.value.offsetWidth;
  const cellHeight = carousel.value.offsetHeight;
  ...
  const cellSize = props.orientation === 'horizontal' ? cellWidth : cellHeight;
