...
export function getHash(str: any): string {
	...
	return str.split('').reduce((hash: number, char: string) => {
...
export function randomSelect(a: Array<string | number>, n = 1): string | number | Array<string | number> {
	if (n === 1) return a[Math.floor(Math.random() * a.length)];
	if (n === -1) n = Math.round(Math.random() * a.length);
...
export class DBmySQL {
	...
	async connectToMySQL() {
		const mysql = require('mysql2/promise') // 引入 mysql2/promise 模块，它支持 Promise API
			, connection = await mysql.createConnection(this.db_config);
		return connection;
	}

	async updateData(table: string, dataset: any, condition: any) {
		const connection = this.db || await this.connectToMySQL();
		try {
			const [result] = await connection.execute(
				`UPDATE ${table} SET column1 = ? WHERE id = ?`,
				['new_value', 1]
			);
			maLogger.log('更新的行数：', result.affectedRows);
		} finally {
	...
	async insertData() {
		...
		try {
			const [result] = await connection.execute(
				...
				['value1', 'value2'] // 参数化查询，防止 SQL 注入
			);
			maLogger.log('插入的行数：', result.affectedRows);
			maLogger.log('插入的 ID：', result.insertId);
		} finally {
			await connection.end();
		}
	}
...
export function randomString(e: number, m = "mixed") {
	e = e || 32;
	var a = "ABCDEFGHJKMNPQRSTWXYZabcdefhijkmnprstwxyz",
		n = "0123456789",
		r = "",
		l = 0;
	switch (m) {
		case "mixed":
			let t = a + n;
			l = t.length;
			for (let i = 0; i < e; i++) r += t.charAt(Math.floor(Math.random() * l));
			break;
		case "number":
			l = n.length
			for (let i = 0; i < e; i++) r += n.charAt(Math.floor(Math.random() * l));
			break;
		case "alphabet":
			l = a.length
			for (let i = 0; i < e; i++) r += a.charAt(Math.floor(Math.random() * l));
			break;
	}
	return r;
}
export function randomNum(Min: number, Max: number, format = false): number | string {
	let res = Min + Math.round(Math.random() * (Max - Min))
	if (format) {
		let tl = String(Max).length;
		let rl = String(res).length;
...
export function generateRandomIDCard(): string {
	...
	const base = provinceCode + cityCode + countyCode + birthYear + birthMonth + birthDay + sequenceCode;

	// 计算校验码
	const weights = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2];
	const checkCodes = ['1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2'];
	const sum = base.split('').reduce((acc, cur, idx) => acc + parseInt(cur) * weights[idx], 0);
...
export function parseCSV(csvContent: string): Array<Record<string, string | null>> {
	try {
		...
		for (let i = 1; i < lines.length; i++) {
			if (lines[i].trim() !== '') {
				...
				for (let j = 0; j < headers.length; j++) {
					obj[headers[j].trim()] = values[j] ? values[j].trim() : data[i - 2][headers[j].trim()] ? data[i - 2][headers[j].trim()]!.trim() : null;
				}
				data.push(obj);
			}
		}
