...
const PREFIXED_CONSOLE_METHODS = new Set<PropertyKey>([
  "debug",
  "error",
  "info",
  "log",
...
const createDebugConsole = (
  ...
  const debugConsole: Console & DEBUGER = new Proxy(baseConsole, {
    get: (target, prop, receiver) => {
      ...
      return (...args: any[]) => {
        Reflect.apply(
          consoleValue,
          target,
          withDebugPrefix(state.title, prop, args),
        );
        return debugConsole;
      };
    },
...
export const parseDomains = (domainsString: string): [string, string][] => {
  ...
  if (domainCache.has(domainsString)) {
    return domainCache.get(domainsString)!;
  }
...
export const equalDomain = (
  domain: [string, string],
  hostname: string,
  port: string,
): boolean =>
  (domain[0] === hostname || domain[0] === "*") &&
  (domain[1] === port || domain[1] === "*");
