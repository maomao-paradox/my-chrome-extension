...
const circleMath = {
  ...
  parseSXY: function (rotation, self) {
    const farScale=self.farScale;
    const itemWidth=self.itemWidth;
    const xs=self.xs; const xr=self.xr; const ys=self.ys; const yr=self.yr;
    const sin = Math.sin(rotation), cos = Math.cos(rotation);
 ...
 * @param id
 */
const Rotation3D = window.Rotation3D = function (_opts) {
  const self=this;
...
Rotation3D.prototype.scheduleNextFrame = function () {
  ...
  const playFrame = function () {
    ...
    const now = time(), dt = (now - self.lastTime) * 0.002;
    self.lastTime = now;
    // console.log('rem',rem)
