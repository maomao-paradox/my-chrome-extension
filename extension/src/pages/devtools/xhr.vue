...
async function saveRule() {
  ...
  try {
    ...
    chrome.devtools.inspectedWindow.eval(`window.XHR_PATCH_MANAGER.updateRules(${payloadStr});`,
      function (result: any, isException) {
        if (isException) {
          maLogger.error('更新规则失败:', result);
        } else {
          // 更新本地规则列表
...
async function toggleXhrPatch() {
  try {
    ...
    isLoading.value = true;
    chrome.devtools.inspectedWindow.eval(`window.XHR_PATCH_MANAGER.toggleEnabled(${newState})`,
      function (result: any, isException) {
        if (isException) {
          maLogger.error('切换XHR补丁状态失败:', result);
          // 发生错误时也回滚UI状态
          isXhrEnabled.value = !newState;
        } else {
          isXhrEnabled.value = result.isEnabled !== undefined ? result.isEnabled : newState;
          isXhrPatched.value = result.isPatched !== undefined ? result.isPatched : (newState && Object.keys(rules.value).length > 0);
        }
      }
    );
  } catch (error) {
    ...
    // 发生错误时也回滚UI状态
    isXhrEnabled.value = !isXhrEnabled.value;
  } finally {
    isLoading.value = false;
  }
}
