...
const Vector2 = function () {
  ...
  _createClass(Vector2, [{
    ...
    value: function midpoint(vec) {
      const mp = new Vector2(this.x + vec.x, this.y + vec.y);
      mp.divideScalar(2);
      return mp;
    }
  }, {
    ...
    value: function angleTo(vec, format) {
      vec = vec || this;
      format = format || 'rad';
      const angle = format === 'rad' ? Math.atan2(vec.y - this.y, vec.x - this.x) : format === 'deg' ? Math.atan2(vec.y - this.y, vec.x - this.x) * 180 / Math.PI : undefined;
      return angle;
    }
  }]);
