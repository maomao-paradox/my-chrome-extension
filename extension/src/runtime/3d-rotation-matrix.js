  ...
  createDot(x = null, y = null, z = null) {
    ...
    return {
      ...
      update: function () {
        ...
        const zPercentage = this.position.z / self.pointsRange;
        this.radius = self.baseDotRadius + ((self.baseDotRadius / 3) * zPercentage);
        this.opacity = 0.5 + ((zPercentage + 1) / 4);
        const depthOfFieldMultiplier = ((zPercentage + 1) / 2) + 0.5;
  ...
  render() {
    ...
    this.dots.forEach((dot, index) => {
      ...
      for (let i = index; i < this.dots.length; i++) {
        // 使用距离公式计算点之间的距离
        const distance = Math.sqrt(Math.pow(this.dots[i].position.x - dot.position.x, 2) + Math.pow(this.dots[i].position.y - dot.position.y, 2) + Math.pow(this.dots[i].position.z - dot.position.z, 2));
