...
async function writeStoredAccounts(
      ...
      return [
        {
          ...
          algorithm: "SHA1",
          createdAt: "2026-07-17T05:52:35.107Z",
          digits: 6,
          id: "totp_1784267555107_qndw09",
          issuer: "Test",
          period: 30,
          secret: "Z64HS6ONTDLY2M6U",
          updatedAt: "2026-07-17T05:52:35.107Z",
        },
      ];
    }
...
function normalizeCreateRequest(
  body: CreateTOTPAccountRequest,
): Required<CreateTOTPAccountRequest> {
...
function normalizeAlgorithm(algorithm?: string): "SHA1" | "SHA256" | "SHA512" {
  const normalized = (algorithm || "SHA1")
    .trim()
    .toUpperCase()
    .replace("-", "");
...
function normalizePeriod(period?: number): number {
  return Number.isFinite(period) && period
    ? Math.max(10, Math.min(300, Math.trunc(period)))
    : 30;
}
...
function isStoredAccount(value: unknown): value is StoredTOTPAccount {
  ...
  return Boolean(
    account?.id && account.accountName && account.secret && account.period,
  );
}
