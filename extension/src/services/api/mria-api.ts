...
export class Requester {
  ...
  constructor(token?: string | null) {
    ...
    try {
      contentStorage = storage.page.local.get(
        "Manteia-UserInfo",
        {},
      ) as SessionInfo;
    } catch (e) {
  ...
  restruct(): void {
    ...
    try {
      const contentStorage: SessionInfo = storage.page.local.get(
        "Manteia-UserInfo",
        {},
      ) as SessionInfo;
      ...
      this.hospital_code =
        contentStorage.hospital_code || "HOSPITSDZL0018465465";
    } catch (e) {
...
export async function reloadRemoteDevices(this: Requester): Promise<void> {
  try {
    const guac_token = await this.mpost("/api/get_remote_guac_token").then(
      (res: any) => res.data?.guac_token,
    );
    ...
    if (device_list && device_list.length > 0) {
      for (const item of device_list) {
        ...
        await this.mpost("/api/add_remote_device", item).then((res: any) =>
          maLogger.log(`重新添加${device_name}：${res.msg}`),
        );
...
export async function updatePatientImage(
  ...
  try {
    ...
    if (!id_number || !patient_code) {
      throw new Error("未获取到患者信息");
    }

    return await this.mpost_file(
      "/mriaapi/update_patient_image",
      {
        id_number: id_number,
...
// 上传签名图片
export async function uploadSignPicture(
  this: Requester,
  img_file: File,
): Promise<any> {
  try {
...
export async function downloadSheet(
  this: Requester,
  form_code: string,
  sheet_component_code: string,
): Promise<void> {
  try {
    const items = await this.mpost("/api/search_sheet_template", {
      form_code: form_code,
      name: "",
    ...
    for (const item of items) {
      try {
        ...
        const response = await this.mget(
          "/api/download_sheet_template",
          params,
          {
...
export async function downloadSheetTemplate(
  ...
  try {
    ...
    const component_code = await findComponentCodeByName(
      components,
      component_name,
    );
...
// 通过名称获取表单代码
export async function getFormCodeByName(
  this: Requester,
  form_name: string,
): Promise<string | null> {
...
// 获取医疗科室列表
export async function getMedicalDepartmentList(
  this: Requester,
): Promise<any[]> {
  return await this.mpost("/api/get_medical_department_list").then(
...
export async function addForm(
  ...
  const dept_code =
    dept_list.length > 0 ? dept_list.map((dp: any) => dp.dept_code) : [];
...
// 获取表单设计
export async function getFormDesign(
  this: Requester,
  form_code: string,
): Promise<any[]> {
...
// 标记高危
export async function markHighRisk(
  this: Requester,
  patient_visit_code: string,
  treat_flow_code: string,
): Promise<any> {
  try {
    const patient_inspect_info = await this.mpost(
      "/mriaapi/get_patient_inspect_info",
      {
    ...
    const node_code = await this.mpost(
      "/mriaapi/get_patient_treatflow_flownode",
      {
...
export async function registerPatient(
  ...
  try {
    const form_code =
      (await getFormCodeByName.call(this, "^基本信息$")) ||
      "FORMXXdzcf0y1CsSmiqTbr1655088868";
    ...
    const form_data = {
      ...data,
      name: patient_name
        ? patient_name.length > 1 << 5
          ? patient_name.substring(1 >> 1, 1 << 5)
          : patient_name
        : "测试患者",
      docGroup:
        doctorList && doctorList.length > 1 >> 1
          ? [(randomSelect(doctorList) as any).user_code]
          : null,
    };

    const components = await getFormDesign
      .call(this, form_code)
      .then((cs: any[]) => writeForm(cs, form_data));

    const head_img_url = head_img
      ? await uploadPatientImage.call(this, head_img)
      : "";
...
// 创建治疗流程
export async function createTreatflow(
  this: Requester,
  patient_visit_code: string,
): Promise<any> {
  try {
    const flow_code =
      (await searchFlow.call(this, "通用流程")) ||
      (await searchFlow.call(this, "完整流程")) ||
      (await searchFlow.call(this, "外照射流程"));

    const diagnoses = randomSelect(
      await getDiagnosis
        .call(this)
        .then((res: any[]) => res.map((d) => d.value)),
    );
    ...
    const form_code =
      (await getFormCodeByName.call(this, "^程.?段信息$")) ||
      "FORMXXmXLKBd2CEekac6vr1670919850";
    const components = await getFormDesign
      .call(this, form_code)
...
// 根据名称查找组件代码
export function findComponentCodeByName(
  components: any[],
  componentTypeName: string,
): string | null {
  function searchComponent(component: any): string | null {
    if (
      component.component_type_name === componentTypeName &&
      component.name ===
        document.body.querySelector("div.active > div > div > span ")?.innerHTML
    ) {
      return component.code;
    }
    if (component.children && component.children.length > 0) {
      for (const child of component.children) {
        const code = searchComponent(child);
        if (code) {
          return code;
        }
      }
  ...
  try {
    for (const component of components) {
      const code = searchComponent(component);
      if (code) {
        return code;
      }
    }
