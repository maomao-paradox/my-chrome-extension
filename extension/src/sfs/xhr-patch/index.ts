...
(function () {
  ...
  const DEBUG_PREFIX = "[XHR-PATCH DEBUG]";
  enum DebugMethod {
    Log = "log",
    Info = "info",
    Warn = "warn",
  ...
  // 创建带分组的日志函数
  const createDebugMethod = (debugMethod: DebugMethod) => {
    return (...args: any[]) => {
      console.groupCollapsed(DEBUG_PREFIX);
      console[debugMethod](...args);
      console.groupEnd();
    };
  };

  const debug = {
    log: createDebugMethod(DebugMethod.Log),
    info: createDebugMethod(DebugMethod.Info),
    warn: createDebugMethod(DebugMethod.Warn),
    error: createDebugMethod(DebugMethod.Error),
  };
  ...
  const xhrPatchConfig: XHR_PATCH_CONFIG = {
    ...
    loadRulesFromStorage: function () {
      try {
          ...
          try {
            ...
            if (storedRules && typeof storedRules === "object") {
              ...
              if (
                this.isEnabled &&
                !this.isPatched &&
                Object.keys(this.rules).length > 0
              ) {
                this.applyPatch();
              }
            }
        ...
        // 静默失败，避免影响页面正常加载
        debug.error("从localStorage加载规则失败:", error);
      }
    },
    ...
    saveRulesToStorage: function () {
      try {
        ...
        // 静默失败，避免影响页面正常加载
        debug.error("保存规则到localStorage失败:", error);
      }
    },
    ...
    updateRules: function (newRules: Record<string, XhrRule>) {
      if (newRules && typeof newRules === "object") {
        ...
        try {
          ...
          debug.error("通知内容脚本更新规则失败:", error);
        }
    ...
    clearRules: function () {
      ...
      try {
        ...
        debug.error("通知内容脚本清空规则失败:", error);
      }
    ...
    applyPatch: function () {
      if (!this.isEnabled) {
        debug.log("[XHR Modifier] XHR patching is disabled, skipping");
        return;
      }

      if (this.isPatched) {
        debug.log("[XHR Modifier] XHR already patched, skipping");
        return;
      }

      if (Object.keys(this.rules).length === 0) {
        debug.log("[XHR Modifier] No rules to apply, skipping patch");
        return;
      }
      ...
      try {
        patchXHR(this.rules);
        this.isPatched = true;
        debug.log("XHR补丁已应用，当前规则数:", Object.keys(this.rules).length);
      } catch (error) {
        debug.error("应用XHR补丁失败:", error);
    ...
    resetPatch: function () {
      if (!this.isPatched) {
        debug.log("[XHR Modifier] XHR not patched, nothing to reset");
        return;
      }

      try {
        debug.log("[XHR Modifier] Resetting XHR patch");
        // 恢复原始的XMLHttpRequest
        if (this.originalXHR) {
  ...
  interface OPERATION_LIBRARY {
    ...
    setStatus: (
      response: any,
      params: { status: number; statusText?: string },
    ) => any;
  }
  ...
  const operationLibs: OPERATION_LIBRARY = {
    ...
    setField: (data: any, params: { path: string; value: any }) => {
      ...
      for (let i = 0; i < pathSegments.length - 1; i++) {
        ...
        if (arrayMatch) {
          ...
          if (!Array.isArray(current[key])) {
            debug.warn(
              `[XHR Modifier] Expected ${key} to be an array but got ${typeof current[key]}`,
            );
            return data;
          }
      ...
      debug.log(`[XHR Modifier] Set field ${path} to ${value}`);
    ...
    //     } catch (e) {
    //         debug.error('[XHR Modifier] Error in replaceResponse:', e);
    //     }
    //     return response;
    // }
  };
    ...
    for (const instruction of instructions) {
      ...
      if (operationLibs[type]) {
        try {
          ...
          debug.error(`[XHR-PATCH] Error executing instruction ${type}:`, e);
        }
      } else {
        debug.warn(`[XHR-PATCH] Unknown operation type: ${type}`);
      }
    }
  ...
  const getApi = (url: string) => {
    ...
    try {
      ...
      debug.error("[XHR-PATCH] Error parsing URL:", e);
      return "";
    }
  };
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          // debug.log(`[XHR-PATCH] executeInstructions response:`, newXhrResponse);
          newXhrResponse.responseText = JSON.stringify(newXhrResponse.response);
        } catch (parseError) {
          // 如果解析失败，使用原始响应
          debug.warn("[XHR-PATCH] Failed to parse JSON response:", parseError);
        }
      }
      ...
      debug.log(
        `[XHR-PATCH] Modified response for ${xhr.responseURL}: `,
        newXhrResponse,
      );
    } catch (e) {
      debug.error("[XHR-PATCH] Error in modifyResponse:", e);
    }
  };
  ...
  const patchXHR = (rules?: Record<string, XhrRule>) => {
    ...
    class NewXHR extends oldXHR {
      constructor() {
        ...
        this.open = function (
          ...
          const currentRules: XhrRulesMap =
            //@ts-ignore 获取最新的规则
          ...
          api = getApi(url);
          if (
            Object.prototype.hasOwnProperty.call(currentRules, api) &&
            ...
            try {
              ...
              debug.error(
                `[XHR-PATCH] Error executing open instructions for ${api}:`,
                e,
              );
            }
          }

          debug.log("[XHR-PATCH] 修改后的请求params:", params);
        ...
        this.send = function (body?: any) {
          ...
          try {
            ...
            if (requestHeaders) {
              debug.log("[XHR-PATCH] requestHeaders: ", requestHeaders);
            }
          } catch (e) {
            // 忽略错误
          }
          ...
          if (Object.prototype.hasOwnProperty.call(currentRules, api)) {
              ...
              try {
                ...
                if (
                  ...
                  try {
                    ...
                    // 如果解析失败，使用原始body
                    debug.warn(
                      "[XHR-PATCH] Failed to parse JSON body:",
                      parseError,
                    );
                  }
                }

                const modifiedBody = executeInstructions(
                  parsedBody,
                  //@ts-ignore 执行send指令
                  currentRules[api].sendRules,
                );
                ...
                debug.error(
                  `[XHR-PATCH] Error executing send instructions for ${api}:`,
                  e,
                );
              }
            }
            if (Array.isArray(currentRules[api].responseRules)) {
              ...
              try {
                ...
                this.onreadystatechange = function () {
                  if (this.readyState === 4) {
                    try {
                      ...
                      debug.error(
                        `[XHR-PATCH] Error modifying response for ${api}:`,
                        e,
                      );
                    }
                  }
                ...
                debug.error(
                  `[XHR-PATCH] Error executing interceptResponse instructions for ${api}:`,
                  e,
                );
              }
            }
          ...
          // 传递可能修改后的body参数
          debug.log("[XHR-PATCH] 修改后的body:", body);
    ...
    for (const key in oldXHR) {
      if (
        oldXHR.hasOwnProperty(key) &&
        typeof (oldXHR as any)[key] !== "function"
      ) {
      ...
      switch (type) {
        ...
        case "TOGGLE_XHR_PATCH":
          ...
          try {
            if (typeof data.enable === "boolean") {
              ...
              debug.log("XHR补丁状态已切换为:", data.enable);
              const status = curXhrPatch.getStatus();
              // 通知页面更新状态
              window.postMessage(
                { id, source: "page", payload: { ...status } },
                window.origin,
              );
            }
          } catch (error) {
            debug.error("切换XHR补丁状态失败:", error);
          }
          break;
        ...
        case "MRIA_XHR_GET_STATUS":
          ...
          try {
            ...
            window.postMessage(
              { id, source: "page", payload: { ...status, rules } },
              window.origin,
            );
          } catch (error) {
            debug.error("获取并返回补丁状态失败:", error);
          }
          break;
      }
    } catch (error) {
  ...
  function initXhrPatch(context: AppContext) {
    ...
    if (Object.keys(xhrPatchConfig.rules).length === 0) {
      debug.log("XHR补丁已就绪，请求规则");
      document.dispatchEvent(
        new CustomEvent("MRIA_XHR_PATCH_READY", {
          detail: {
            version: "1.0",
