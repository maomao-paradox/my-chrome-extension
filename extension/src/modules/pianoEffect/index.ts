 ...
 * @license MIT
 * @sequence X
 * @file src/modules/pianoEffect/index.ts
...
class PianoEffect implements AppModule {
  ...
  private async preloadSounds(): Promise<void> {
    try {
      ...
      const preloadPromises = commonKeys.map(async (key) => {
        ...
        if (
          soundFile &&
          !this.soundBuffers.has(soundFile) &&
          this.audioContext
        ) {
          try {
            const { buffer, ok } = await fetchAndDecodeAudioBuffer(
              soundFile,
              this.audioContext,
            );
            if (ok && buffer) {
  ...
  private async playSound(key: string): Promise<void> {
    ...
    // 初始化上下文并恢复状态
    if (!(await this.ensureAudioContextReady())) {
      return;
    }
  ...
  public async init(): Promise<void> {
    try {
      ...
      // 从本地存储加载设置
      const enablePiano = await storage.ext.local.get(
        "enablePianoEffect",
        false,
      );
