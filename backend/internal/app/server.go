...
func NewServer(cfg config.Config, logger *slog.Logger) http.Handler {
	...
	mux.HandleFunc("POST /api/automation/tasks", handler.CreateAutomationTask)
	mux.HandleFunc("GET /api/automation/tasks/{id}", handler.GetAutomationTask)
	mux.HandleFunc("POST /api/automation/tasks/{id}/steps", handler.SaveAutomationTaskSteps)
	mux.HandleFunc("POST /api/automation/tasks/{id}/runs", handler.CreateAutomationRun)
	mux.HandleFunc("POST /api/automation/runs/{id}/events", handler.CreateAutomationRunEvent)
	mux.HandleFunc("POST /api/automation/runs/{id}/screenshots", handler.CreateAutomationRunScreenshot)
	mux.HandleFunc("POST /api/automation/generate", handler.GenerateAutomationSteps)
	...
	mux.HandleFunc("GET /api/totp/accounts", handler.ListTOTPAccounts)
	mux.HandleFunc("POST /api/totp/accounts", handler.CreateTOTPAccount)
	mux.HandleFunc("PATCH /api/totp/accounts/{id}", handler.UpdateTOTPAccount)
	mux.HandleFunc("DELETE /api/totp/accounts/{id}", handler.DeleteTOTPAccount)
	mux.HandleFunc("POST /api/totp/accounts/{id}/code", handler.CreateTOTPCode)
